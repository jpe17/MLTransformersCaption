...
class VisionLanguageEncoderWithModality(VisionLanguageEncoderBase):
    """
    Version with modality embeddings - clean implementation based on v2_01 structure
    but includes modality embeddings to distinguish between image and text tokens
    """
...
def train_v2_00_with_modality():
    """
    V2_00: Training with modality embeddings - clean version
    """
    wandb.init(project="image-captioning-v2-experiments", name="v2_00_with_modality", config={
        "learning_rate": 1e-5,
        "weight_decay": 0.01,
        "num_epochs": 5,
        "grad_clip_norm": 1.0,
        "model": "v2_00_with_modality"
    })
    config = wandb.config
    
    device = "cuda" if torch.cuda.is_available() else "cpu"
    print(f"Using device: {device}")
    print("V2_00: Training WITH modality embeddings")
    ...
    with torch.no_grad():
        for i, (pil_images, true_captions) in enumerate(itertools.islice(val_batches, 5)):
            ...
            for _ in range(30): # Max caption length
                ...
                # Add text modality embedding (this is the key difference from v2_01)
