...
class CaptionDecoderEmbeddingsOnly(CaptionDecoderBase):
    """
    Version that only unfreezes QWEN embeddings (not layers)
    """
    def __init__(self):
        super().__init__()
        
        # Freeze ALL of Qwen layers
        ...
        if hasattr(self.qwen_model, 'model') and hasattr(self.qwen_model.model, 'embed_tokens'):
            for param in self.qwen_model.model.embed_tokens.parameters():
                param.requires_grad = True
            print("✅ QWEN embeddings unfrozen")
        elif hasattr(self.qwen_model, 'transformer') and hasattr(self.qwen_model.transformer, 'wte'):
            for param in self.qwen_model.transformer.wte.parameters():
                param.requires_grad = True
            print("✅ QWEN embeddings unfrozen")
        else:
            ...
            for name, param in self.qwen_model.named_parameters():
                if 'embed' in name.lower() or 'wte' in name.lower():
                    param.requires_grad = True
                    print(f"✅ Unfrozen embedding parameter: {name}")

def train_v2_04_embeddings_only():
    """
    V2_04: Training with only QWEN embeddings unfrozen
    """
    wandb.init(project="image-captioning-v2-experiments", name="v2_04_embeddings_only", config={
        "learning_rate": 1e-5,
        "weight_decay": 0.01,
        "num_epochs": 3,
        "grad_clip_norm": 1.0,
        "model": "v2_04_embeddings_only"
    })
    config = wandb.config
    
    device = "cuda" if torch.cuda.is_available() else "cpu"
    print(f"Using device: {device}")
    print("V2_04: Training with only QWEN EMBEDDINGS unfrozen")
    ...
    decoder = CaptionDecoderEmbeddingsOnly().to(device)
    
    # --- 2. Optimizer & Scheduler Setup ---
    # Train image adapter, modality embeddings, cross-attention, and QWEN embeddings only
    ...
    # Add QWEN embedding parameters
    for param in decoder.qwen_model.parameters():
